...
| render                        | 控制单元格自定义渲染                                       | (text: TableDataItem[TableColumnItem[dataKey]], row: TableColumnItem, index: number, dataKey: string) => ReactNode | -                            | -      |
...
| onFilterDropdownVisibleChange | 自定义筛选下拉选项显示状态改变时的回调方法                 | (filterDropdownVisible: boolean, column: TableColumnItem) => void                                                  | true \| false                | false  |
