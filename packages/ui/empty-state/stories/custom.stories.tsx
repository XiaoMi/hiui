import React from 'react'
import EmptyState, {
  EMPTY_STATE_IMAGE_NO_MESSAGE_COLOURFUL,
  EMPTY_STATE_IMAGE_NO_RESULT_COLOURFUL,
  EMPTY_STATE_IMAGE_NO_COLLECTION_COLOURFUL,
  EMPTY_STATE_IMAGE_NO_ACCESS_COLOURFUL,
  EMPTY_STATE_IMAGE_NO_NETWORK_COLOURFUL,
  EMPTY_STATE_IMAGE_NO_OPEN_COLOURFUL,
  EMPTY_STATE_IMAGE_404_COLOURFUL,
  EMPTY_STATE_IMAGE_SERVICE_ERROR_COLOURFUL,
  EMPTY_STATE_IMAGE_PAGE_ERROR_COLOURFUL,
  EMPTY_STATE_IMAGE_SUCCESS_COLOURFUL,
  EMPTY_STATE_IMAGE_ERROR_COLOURFUL,
  EMPTY_STATE_IMAGE_WRANING_COLOURFUL,
  EMPTY_STATE_IMAGE_NO_DATA_COLOURFUL,
} from '../src'

/**
 * @title 自定义
 */
export const Custom = () => {
  return (
    <>
      <h1>Custom</h1>
      <div className="empty-state-custom__wrap">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', rowGap: 32 }}>
          <EmptyState title="无权限" indicator={EMPTY_STATE_IMAGE_NO_ACCESS_COLOURFUL} />
          <EmptyState title="无网络" indicator={EMPTY_STATE_IMAGE_NO_NETWORK_COLOURFUL} />
          <EmptyState title="建设中" indicator={EMPTY_STATE_IMAGE_NO_OPEN_COLOURFUL} />
          <EmptyState title="404" indicator={EMPTY_STATE_IMAGE_404_COLOURFUL} />
          <EmptyState
            title="服务器失去链接"
            indicator={EMPTY_STATE_IMAGE_SERVICE_ERROR_COLOURFUL}
          />
          <EmptyState title="页面出错" indicator={EMPTY_STATE_IMAGE_PAGE_ERROR_COLOURFUL} />
          <EmptyState title="提交成功" indicator={EMPTY_STATE_IMAGE_SUCCESS_COLOURFUL} />
          <EmptyState title="错误提示" indicator={EMPTY_STATE_IMAGE_ERROR_COLOURFUL} />
          <EmptyState title="警告提示" indicator={EMPTY_STATE_IMAGE_WRANING_COLOURFUL} />
          <EmptyState title="暂无数据" indicator={EMPTY_STATE_IMAGE_NO_DATA_COLOURFUL} />
          <EmptyState title="无查询结果" indicator={EMPTY_STATE_IMAGE_NO_RESULT_COLOURFUL} />
          <EmptyState title="暂无收藏" indicator={EMPTY_STATE_IMAGE_NO_COLLECTION_COLOURFUL} />
          <EmptyState title="无消息" indicator={EMPTY_STATE_IMAGE_NO_MESSAGE_COLOURFUL} />
          <EmptyState
            title="自定义图标"
            indicator={
              <img src="data:image/png;base64,"></img>
            }
          />
        </div>
      </div>
    </>
  )
}
