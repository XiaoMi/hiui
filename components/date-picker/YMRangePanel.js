...
class YMRangePanel extends Component {
  ...
  _generatePanelData() {
    ...
    // 组件内左右面板中日期
    const leftDate = getStartDate(date)
    const { year: sYear } = deconstructDate(leftDate)
    const rightDate = isValid(endDate) ? new Date(endDate) : new Date(sYear + '')
      ...
      <div className="hi-datepicker__header">
        {
          <div className="hi-datepicker__header-btns">
            <span onClick={() => this.changeYear(true, lr)}>
              <Icon name="double-left" />
            </span>
          </div>
        }
        <span className="hi-datepicker__header-text">{this.getHeaderCenterContent(year)}</span>
        {
          <div className="hi-datepicker__header-btns">
            <span onClick={() => this.changeYear(false, lr)}>
  ...
  getYearOrMonthData(year, type, _year) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      for (let j = 0; j < 3; j++) {
        ...
        col.value = y
        const _val = pType === 'yearrange' ? new Date(y + '') : new Date(year, y - 1)
  ...
  render() {
    ...
    const bodyCls = classNames('hi-datepicker__body', 'hi-datepicker__body--range')
    return (
