          ...
          menus.push(
            ...
            >
              ...
              {_currentOptions.map((option, index) => {
                ...
                const optionValues = option.jointOption
                  ? optionValue
                  : getOptionValues(value, optionValue, deep) // jointOption为true代表搜索拼接出来的option，直接取value即可
                ...
                return (
                  <li
                    className={classNames('hi-cascader-menu__item', {
                      ...
                      'hi-cascader-menu__item--isFiltered':
                        isFiltered && !option['hightlight'],
                      'hi-cascader-menu__item--path':
                        isFiltered && value.includes(option.id)
                    })}
                    onClick={e => {
                      e.stopPropagation()
                  ...
                  >
                    <span
                      className={classNames(`hi-cascader-menu__item--label`)}
                    >
                      {isFiltered && option['hightlight']
                        ? (option['hightlight'] || []).map((item, index) => (
    ...
    return (
      ...
      >
        ...
        {value.length === 0 && isFiltered && (
          <>'           '<img src={EmptyPng} />'           '<p className='hi-cascader-menuOutter--emptyText'>
