...
class Row extends Component {
  ...
  render () {
    const { expand, k, parent, highlightRows } = this.props
    let classNames = {
      'table-row': true,
...
export default class Body extends Component {
      ...
      if (item.expand) {
        ...
        tr = expand ? (
          <td
            ...
            className={prifix(
              'table-col',
              highlightCols.includes(item.key) ? 'picked' : null
            )}
          >
          ...
          if (parseInt(rowSpan) !== 0) {
            tr.push(
              <td
                className={prifix(
                  'table-col',
                  highlightCols.includes(obj.key) ? 'picked' : null
                )}
                rowSpan={rowSpan}
                colSpan={colSpan}
                data-span={colSpan}
  ...
  getPrefixNodes () {
    const { columns, advance } = this.props
      ...
      return (
        <tr className={prifix('table-row')} key={`suf-${index}`}>
          {columns.map((item, j) => {
            return (
              <td
                className={prifix('table-col')}
                key={`suf-${item.dataIndex}-${index}-${j}`}
              >
                {suf[item.dataIndex]}
              </td>
            )
