...
class Table extends Component {
  ...
  getScrollXContent () {
      ...
      let onRight =
        Math.abs(
          e.target.scrollWidth -
            e.target.scrollLeft -
            parseInt(getStyle(e.target, 'width'))
        ) < 2
    ...
    if (leftFiexColumns.length === 0 && rightFixColumns.length === 0) {
          ...
          <div className={prifix('table-body')} style={{ overflowX: 'auto' }}>
            <TableContent
              ...
              {...Object.assign(
                ...
                { ...props },
                { columns },
                { dataSource, highlightCols, highlightRows },
                { parent: this, cbs: this.cbs, fetch: this.fetch, t: this },
                { headerColumns, name: this.props.name }
              )}
            />
          </div>
      ...
      if (leftFiexColumns.length > 0) {
        scrollTable.push(
          <div
            className={prifix('table-fixed-left')}
            ref={this.fixLeft}
            style={{ display: 'none' }}
            key='left'
          >
                ...
                <TableContent
                  ...
                  {...Object.assign(
                    ...
                    { ...props },
                    {
                      highlightRows,
                      parent: this,
                      columns: leftFiexColumns,
                      headerColumns,
                      name: this.props.name
                    },
                    { dataSource, highlightCols },
                    { cbs: this.cbs, fetch: this.fetch, t: this }
                  )}
                />
              </div>
            </div>
      ...
      if (rightFixColumns.length > 0) {
        scrollTable.push(
          <div
            className={prifix('table-fixed-right')}
            ref={this.fixRight}
            key='right'
          >
            <div className={prifix('table-outer')}>
    ...
    return <div className={prifix('table-content')}>{scrollTable}</div>
  }
    ...
    return (
      <div className={prifix('table-content')}>
        <div className={prifix('table-scroll')}>
          ...
          <div
            className={prifix('table-body')}
            style={{ maxHeight: scroll.y + 'px', overflow: 'auto' }}
          >
            <TableContent
              {...Object.assign(
                {},
  ...
  componentDidUpdate () {
    ...
    let rightFixTable = this.dom.current.querySelectorAll(
      '.hi-table-fixed-right table tr'
    )
    let scrollTable = this.dom.current.querySelectorAll(
      '.hi-table-scroll table tr'
    )
      ...
      hiTableBody &&
        ...
        hiTableBody.forEach(item => {
          ...
          item.onscroll = e => {
            ...
            let fixTableBodyArr = this.dom.current.querySelectorAll(
              '.hi-table-inner'
            )
            fixTableBodyArr &&
              fixTableBodyArr.length > 0 &&
              fixTableBodyArr.forEach(fixTableBody => {
                ...
                fixTableBody.querySelector(
                  'thead'
                ).style.transform = `translate(0,${item.scrollTop}px)`
              })
          }
        })
    }
  ...
  render () {
    ...
    return (
      ...
      >
        {header && (
          <div className={prifix({ 'table-pre-header': true })}>{header()}</div>
        )}
        <div className={prifix({ 'table-container': true })}>
          ...
          {name && (
            <div className={prifix('table-setting')} ref={this.setting}>
              ...
              {columnMenu && (
                <ClickOutside
                  onClickOutside={e => this.setState({ columnMenu: false })}
                >
        ...
        {footer && (
          <div className={prifix({ 'table-pre-footer': true })}>{footer()}</div>
        )}
        {(pagination || columns) && <br />}
        {pagination && (
          <div style={{ display: 'flex', justifyContent: pagePosition }}>
            {
        ...
        {serverPaginationConfig && (
          <div
            style={{ display: 'flex', justifyContent: serverPagePosition }}
            a='1'
          >
            {
              <div className={prifix('table-page')}>
                <Pagination
                  {...serverPaginationConfig}
                  onChange={current => {
                    serverPaginationConfig.onChange &&
  ...
  xscroll () {
    ...
    if (thead) {
      if (
        ...
        fixTop <
          parseInt(getStyle(dom, 'height'), 10) -
            parseInt(getStyle(thead[0], 'height'), 10)
      ) {
        thead.forEach(th => {
        ...
        thead.forEach(th => {
          ...
          if (name) {
            ...
            let h =
              parseInt(getStyle(dom.querySelector('thead'), 'height')) + 'px'
  ...
  getColumns (columns) {
    ...
    const rightFixColumns = columns.filter(
      ({ fixed }) => !!fixed && fixed === 'right'
    )
    const commanColumns = columns.filter(
      ({ fixed }) => !fixed || (fixed !== 'left' && fixed !== 'right')
    )
            ...
            <Checkbox
              ...
              onChange={(e, checked) => {
                ...
                onChange(
                  _selectedRowKeys,
                  data.filter(record =>
                    _selectedRowKeys.includes(record[dataName])
                  )
                )
              }}
            />
  ...
  setColumnsDetail (bool, prop, c) {
    ...
    let leftFiexColumns = []
    let rightFixColumns = []
    let [headerColumns, columns] = this.getHeaderGroup(
      c || props.columns,
      props.rowSelection
    )
  ...
  componentWillReceiveProps ({ data, columns, width, scroll, ...props }) {
    ...
    if (props.origin) {
      ...
      if (
        this.state.serverPagination &&
        this.state.serverPagination.current !== 1
      ) {
