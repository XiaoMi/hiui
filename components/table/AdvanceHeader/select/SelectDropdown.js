...
const SelectDropdown = ({
    ...
    (text = '', uniqueKey) => {
      ...
      return searchbarValue.length > 0 ? (
        <p key={uniqueKey}>
          {parts.map((part, i) =>
            part === searchbarValue ? (
              <span key={i} className={'hi-table-advanceheader-select__dropdown--item__name-hightlight'}>
                {part}
              </span>
            ) : (
              part
  ...
  const renderOption = (isSelected, item) => {
    ...
    return (
      <React.Fragment>
        {mode === 'multiple' && (
          <Checkbox
            className="hi-table-advanceheader-select__dropdown--item__checkbox"
            checked={isSelected}
            disabled={item[transKeys(fieldNames, 'disabled')]}
          >
            <div className="hi-table-advanceheader-select__dropdown--item__name" style={style}>
              {isByRemoteSearch
  ...
  const groupItem = (filterGroupItem, filterItemsIndex) => {
    ...
    const label = (
      <li
        className="hi-table-advanceheader-select__dropdown--label hi-table-advanceheader-select__dropdown--item"
  ...
  const normalItem = (item, filterItemsIndex, isChildItem) => {
    ...
    return (
      <li
        className={classNames('hi-table-advanceheader-select__dropdown--item', `theme__${theme}`, {
          'is-active': isSelected,
          'is-disabled': isDisabled,
          'hi-table-advanceheader-select__dropdown--item--child': isChildItem,
          'is-focus': targetByKeyDown.current && filterItemsIndex === focusedIndex,
          'hi-table-advanceheader-select__dropdown--item-default':
  ...
  const renderItems = () => {
    return (
      <ul
        className="hi-table-advanceheader-select__dropdown--items"
        ref={dropdownWrapper}
        onMouseMove={() => {
      ...
      >
        ...
        {matched === 0 && (
          <li
            className="hi-table-advanceheader-select__dropdown--item hi-table-advanceheader-select__dropdown-item--empty is-disabled"
  ...
  return (
    <div className="hi-table-advanceheader-select__dropdown" style={style}>
      {(searchable || isByCustomSearch) && (
        <div className="hi-table-advanceheader-select__dropdown__searchbar">
          <div className="hi-table-advanceheader-select__dropdown__searchbar--content">
            <span style={{ cursor: 'pointer' }}>
              <Icon name="search" />
            </span>
            <input
              className="hi-table-advanceheader-select__dropdown__searchbar--input"
            ...
            {searchbarValue.length > 0 ? (
              <span style={{ cursor: 'pointer' }} onClick={cleanSearchbarValue}>
                <i className={`hi-icon icon-close-circle hi-table-advanceheader-select__dropdown--icon__close`} />
              </span>
            ) : null}
          </div>
        </div>
      ...
      {loading && (
        <div className="hi-table-advanceheader-select__dropdown--loading">
          <Loading size="small" />
        </div>
      )}
      ...
      {mode === 'multiple' && (showCheckAll || showJustSelected) && (
        <div className={`hi-table-advanceheader-select__dropdown-check-all theme__${theme}`}>
          <div>
            {showCheckAll && (
              <Checkbox
                checked={isCheckAll}
