...
const InternalSelect = (props) => {
  ...
  const handleKeyDown = useCallback(
    (evt) => {
      if (dropdownShow && !disabled) {
        ...
        if (
          evt.keyCode === 32 &&
          !document.activeElement.classList.value.includes('hi-table-advanceheader-select__dropdown__searchbar--input')
        ) {
          evt.preventDefault()
  ...
  return (
    <div
      className={classNames('hi-table-advanceheader-select', className, extraClass)}
      style={style}
      tabIndex="0"
    ...
    >
      <div
        className="hi-table-advanceheader-select__input-container"
      ...
      <Popper
        ...
        className="hi-table-advanceheader-select__popper"
        onKeyDown={handleKeyDown}
        tabIndex="-1"
        width={optionWidth}
        onClickOutside={() => {
