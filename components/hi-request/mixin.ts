...
import type { HiRequestType, HiRequestOptions, HiRequestMethod, HiRequestStaticAxios, HiRequestStatic } from './type'
...
const AXIOS_METHODS: HiRequestMethod[] = ['get', 'post', 'delete', 'put', 'patch', 'head', 'options']

AXIOS_METHODS.forEach((method) => {
  // @ts-ignore
  HiRequest[method] = (url: string, options?: HiRequestOptions) => HiRequest({ ...options, method, url })
})

// 扩展的 axios 静态方法：cancels、all\spread
const AXIOS_REST_STATIC: HiRequestStaticAxios[] = ['CancelToken', 'Cancel', 'isCancel', 'all', 'spread']
...
// jsonp
HiRequest.jsonp = (url: string | HiRequestOptions, options?: HiRequestOptions) => InternalRequest('jsonp', url, options)
