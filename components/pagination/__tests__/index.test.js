  ...
  describe('PropTypes', () => {
    it('type', () => {
      ...
      const wrapper = mount(
        <div>
          {
            types.map((type, index) => <Pagination total={100} pageSize={10} type={type} key={index} />)
          }
        </div>
      )
      types.map(type => {
    ...
    it('defaultCurrent', () => {
      const dc = 2
      const wrapper = mount(
        <Pagination total={100} pageSize={10} defaultCurrent={dc} />
      )
    ...
    it('current', () => {
      const dc = 2
      const wrapper = mount(
        <Pagination total={100} pageSize={10} current={dc} />
      )
      expect(wrapper.find(`.hi-pagination__item-${dc}`).hasClass('hi-pagination__item--active')).toBeTruthy()
    ...
    it('max', () => {
      // TODO: max 属性
      const wrapper = mount(
        <Pagination total={100} pageSize={10} max={5} />
      )
      wrapper.setProps({current: 3})
      expect(wrapper.find(`.hi-pagination__item-3`).hasClass('hi-pagination__item--active')).toBeTruthy()
    })
    ...
    it('pageSizeOptions && onPageSizeChange', () => {
      const wrapper = mount(
        <Pagination total={100} pageSize={10} pageSizeOptions={[10, 20, 50]} />
      )
