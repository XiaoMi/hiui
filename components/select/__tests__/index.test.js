...
describe('Select', () => {
  ...
  it('should control by value', () => {
    types.forEach((type)=>{
      ...
      if (type === 'multiple') {
        ...
        expect(wrapper.find('.hi-select__input').find('.hi-select__input--item').map(val => val.find('.hi-select__input--item__name').text())).toEqual(['笔记本', '生活周边', '其它'])
      }
      wrapper.unmount()
    })
  ...
  it('should handle keydown', () => {
    ...
    wrapper.find('.hi-select__input').simulate('click')
    expect(document.querySelectorAll('.hi-select__dropdown--item')[1].getAttribute('data-focused')).toEqual("false")
  ...
  it('异步单选', (done) => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      'http://yapi.demo.qunar.com/mock/26534/hiui/select?id=1',
      {
        'method': 'GET',
      }
    )
  ...
  it('异步多选首次加载数据', (done) => {
    ...
    expect(global.fetch).toHaveBeenCalledTimes(1)
    expect(global.fetch).toHaveBeenCalledWith(
      'http://yapi.demo.qunar.com/mock/26534/hiui/select?',
      {
        'method': 'GET',
      }
    )
