...
受管筛选链路还必须证明 `queryFields` 的最终字段角色没有漂移：关键词位保持 `search-input` 语义，
普通文本筛选字段保持 `filter-text-input` 角色并与关键词搜索共享同一筛选皮肤，`Select / DatePicker`
等字段继续落在共享 `query-filter` 表面。`query-filter-guard` 不能只检查“字段数量还在”，还要检查这些
字段是否仍命中 `queryFieldRenderProfile` / `filterSurfaceBaseline`。
...
- 普通文本筛选字段静默回退成裸 `Input`、`schema-fields Text()` 未命中受管 `filter-text-input` 角色、
  或 `SearchInput` 灰底仍在但其它文本 / 选择 / 日期字段漂移成另一套白底 / line 皮肤时，按
  `query filter field profile drift` 处理；strict rollout / managed list 主链下至少 P1，正式验收与新增页按 P0 失败。
- 页面把多个普通文本筛选字段一律伪装成 `SearchInput` 以维持灰底外观，导致字段角色退化成多个“主搜索入口”时，
  按 `query filter field role drift` 处理；修复方向是恢复共享皮肤下的独立 `filter-text-input` 角色，而不是继续堆叠搜索语义。
...
### Managed Analytics Control Strip Lock

对 `data-visualization`，`query-filter` region 默认不再等价于列表页的 `QueryFilter`。当
`semanticContract.queryFilterRegionRole=dashboard-control-strip` 时，它代表页面级视角 / 时间粒度 /
分析维度切换，影响范围覆盖指标卡、图表与明细表；因此必须作为白底主体最上方的独立控制行存在，而不是
被塞回筛选表单、灰底 panel 或与明细 `QueryFilter` 混成同一行。

硬门槛：

- `dashboard-control-strip` 没有保持 `top-of-white-body-before-stat-section`，而是出现在 `stat-section`
  或 `chart-section` 之后，P0 失败。
- `dashboard-control-strip` 被背景色、描边、圆角、阴影等 panel chrome 包裹，未保持
  `plain-row-no-panel`，P0 失败。
- 页面级控制条与明细 `QueryFilter` 共处同一 row / carrier，或 `QueryFilter` 被塞进
  `DashboardControlStrip`，P0 失败。
- 当 contract 声明 `detailQueryFilterPolicy=separate-detail-query-filter-near-detail-table` 时，
  允许存在独立明细 `QueryFilter`；但它必须在 `chart-section` 之后、明细表之前，紧邻 detail table。
  若提前到分析区之前，或漂移到明细表之后，P0 失败。
- `dashboard-control-strip` 仍通过 `QueryFilter queryFields` 承接日 / 周 / 月、分析视角等页面级切换，
  P0 失败；这些控制必须保持 `Radio.Group` / `Tabs` / `Segmented` 等 control-strip 语义。

### Managed Body Section Contract Lock

对 `full-page-edit`、`drawer-form`、`drawer-detail`、`full-page-detail`，外壳正确并不等于页面完成。
当业务页在 `form-body` / `detail-body` 内新增一级 section、独立 widget、只读摘要、媒体行或局部工具区时，
必须继续落在受管的 body section 语义里，而不是回退成“壳内自由布局”。

硬门槛：

- 命中上述 4 个页型但 contract 缺少 `semanticContract.bodySectionContract`，P0 失败。
- `bodySectionContract` 的 `primaryExpression`、`sectionComposition`、`sectionSpacingOwnership`、
  `sectionContainerPolicy`、`embeddedWidgetPolicy` 任一缺失或值域非法，P0 失败。
- `form-body` / `detail-body` 中出现一级 section 扩展、独立表格、只读图表摘要、媒体行或控制条，但
  没有对应 `supportingSections` / page-component `allowedExtensions` / `embeddedWidgetPolicy` 声明，P0 失败。
- body 一级 section 通过自由 wrapper、自写 grid、自写 panel、自建局部 workspace 或第二层白底主体组织，
  导致 contract 无法解释其 owner 与作用域时，按 `HIUI046` / `HIUI049` 处理，P0 失败。
- body 一级 section 的纵向节奏退回 page root、field grid、统一尾留白或其它错误 owner，而不再由
  section 结构、`FormItem` 或 `Descriptions` group 承担时，按 `HIUI047` 处理，P0 失败。
- `full-page-edit` / `drawer-form` 默认只允许 `groups-only`；若出现独立 supporting section，必须显式升级为
  `groups-with-supporting-sections`，但不得接管 `white-body`、`main-scroll`、`query-filter`、`pagination`
  或 `footer` owner。
- `drawer-detail` / `full-page-detail` 默认只允许 `Descriptions` 主表达；新增 supporting section 时，
  仍必须保持 `Descriptions` / `SchemaDescriptionsBridge` 为主表达，不能退化成字段卡、panel 列表或第二套摘要壳。
...
`legacy-host-compatible` 普通典型页的硬门禁编排以 `rules/legacy-host-hard-gates.json` 为唯一机器真相；本文件只定义治理分层与阻断原则，不再重复手写 legacy hard gate 数量。
...
function ManagedFilterTextInput(props) {
  return <Input appearance="filled" {...props} />
}
...
`ManagedFilterTextInput` 只是字段角色示意：它强调“与 `ManagedSearchInput` 共享筛选皮肤，但不冒充搜索语义”。
强制 baseline 不得被业务 props 关闭；需要覆盖时必须 contract 记录 exception、owner、expiry。

对页面生成链路，`QueryFilter` 字段基线应优先收敛到统一字段工厂，而不是继续在业务页临时拼
`Input / SearchInput / Select / DatePicker`：
...
<TablePageFrame searchPlaceholder="用户名称 / 米聊 / 电话" queryFields={queryFields} />
```

这里的约束是：

- 页框内置关键词检索位继续走 `searchPlaceholder` 链路。
- 普通文本筛选位统一走 `createManagedQueryTextField` / `filter-text-input`。
- `Select / DatePicker` 分别统一走 `createManagedQuerySelectField`、`createManagedQueryDateRangeField`，继续落在共享 QueryFilter 表面，不得混入另一套白底 / line 皮肤。
...
- `query-filter-guard` 还要防字段角色漂移与筛选皮肤割裂。
