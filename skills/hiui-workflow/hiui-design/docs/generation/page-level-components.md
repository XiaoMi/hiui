...
legacy 项目不要求在“项目接入完成”时覆盖所有典型页型；但 legacy 接入完成这个定义本身要求先完成一批 `carrier-first-required` 的 project rollout：`table-basic`、`table-stat`、`tree-table`、`tree-split`、`drawer-form`、`drawer-detail`、`full-page-edit`、`full-page-detail`。`feedback-status` 与 `data-visualization` 可以延后，但它们不应反向稀释这批表格 / 编辑 / 详情页的 carrier-first onboarding 事实。换句话说，若 required batch 仍缺 carrier，应视为 legacy 接入 / bootstrap 尚未完成，而不是“已接入后继续补 rollout”。
