...
### Legacy Bridge Ownership Hardening

这组规则只收敛 `legacy-host-compatible` 下的 `page-component + runtime bridge + slot fill` 主链，不把旧宿主桥接问题扩散成全模式通用复杂度。

默认要求：

- selected page component / project-certified carrier 必须继续承接 `white-body`、`main-scroll`、`pagination` 与真实 `QueryFilter` carrier。
- bridge wrapper、slot adapter 与业务页本地 wrapper 不得再合成第二层 `white-body shell`、`main-scroll shell`、`pagination shell` 或 `query shell`。
- 若页面存在主体导航 Tabs，必须保持在白底主体顶部并位于 `QueryFilter` 之前；只有 contract 显式声明“Tabs 替代标题”时，Tabs 才能进入 header region。
- legacy 表格类页面的样式表象不得再由业务页本地 CSS 兜底。搜索框灰底、筛选控件统一皮肤、排序箭头密度、分页白底承接与白底内部滚动，统一视为 shared carrier / style boundary 证据，而不是局部视觉补丁点。
- 这组问题在 legacy `migration / managed / release` profile 下按 blocking 处理；`host-integration` / `rules-only` 不因为这批 drift signal 自动升级新的 hard fail。

稳定 drift signals：

- `WHITE_BODY_OWNER_DRIFT`
- `MAIN_SCROLL_OWNER_DRIFT`
- `PAGINATION_MOUNT_DRIFT`
- `TOP_NAV_REGION_DRIFT`
- `QUERY_FILTER_CARRIER_DRIFT`
- `LEGACY_STYLE_BOUNDARY_DRIFT`

其中 `LEGACY_STYLE_BOUNDARY_DRIFT` 不再拆成多条并行 failure code，而是统一挂以下 evidence ids：

- `search-input-filled`
- `query-control-skin-consistency`
- `sort-icon-density`
- `pagination-surface`
- `white-body-scroll-internal`
...
对 `legacy-host-compatible` 的 bridge ownership hardening，第一批核心 DOM 断言收敛为：

- `white-body` 才是滚动 owner，而不是全局 page root
- `bodyTopNavigation` 位于白底主体内，且在 `QueryFilter` 之前
- 若 Tabs 替代页面标题进入 `header`，必须显式声明 header-tabs 变体，并保持 `Tabs size=lg`
- `pagination` 位于 `white-body / managed list workspace` 内部
- `header` 位于主滚动链之外，不被 `main-scroll` 包进主体滚动容器（`UX004_HEADER_OUTSIDE_MAIN_SCROLL`）

在上述 DOM 断言之外，`legacy-host-compatible` 的表格类页面若命中 `styleBoundary` 风险信号，还必须补一层 computed-style probe。第一批 probe evidence ids 固定为：

- `search-input-filled`：搜索输入框必须保留 QueryFilter filled 灰底，不得退化成透明或白底输入框
- `query-control-skin-consistency`：QueryFilter 内部控件皮肤必须保持同一套 surface 基线，不得一部分像输入框、一部分像裸文本
- `sort-icon-density`：表头排序上下箭头必须保持紧凑密度，不得被外部样式拉大间距
- `pagination-surface`：分页必须继续承接同一白底 workspace，并贴住 managed list workspace 底部
- `white-body-scroll-internal`：滚动条必须留在白底主体内部，页头不得跟随主体滚动

这 5 个 evidence 统一归档到 `LEGACY_STYLE_BOUNDARY_DRIFT`，用于表达“shared style boundary 漂移”，而不是继续扩成新的平行 failure code。
