...
| `HIUI022_LEGACY_FEATURE_REGRESSION` | governance/runtime | 页面升级后原有关键功能、关键区块或关键业务动作丢失，且缺少带 provenance 的批准记录 | 升级结果回归了原页面能力，不能按“只是视觉重构”解释 | 恢复缺失能力，或补齐显式批准、替代方案与 provenance | 部分 | 是 |
| `HIUI023_UNAPPROVED_STRUCTURE_DELTA` | governance/preflight/runtime | 页面升级发生未审批的一级分组重排、阅读顺序变化、主辅关系变化、工作区拓扑变化或页型退化 | 页面结构已经超出允许的升级边界 | 回退未批准的结构改动，或补齐明确批准与新的 contract / acceptance 证据 | 部分 | 是 |
| `HIUI024_CONTENT_OVERFLOW_OR_CLIPPING` | governance/runtime | 文本、表格、图表、关键操作、抽屉、弹窗或浮层出现不可接受的溢出、裁切、遮挡或宿主层级裁切 | 页面内容完整性被破坏，用户已无法稳定阅读或操作 | 修复布局几何、host container、popup container、scroll owner 或响应式断点后重跑验证 | 部分 | 是 |
| `HIUI025_VISUAL_DECORATION_DRIFT` | governance/preflight/runtime | 页面引入无业务语义、无规则依据的投影、渐变、发光、纹理、装饰性背景或纯视觉 filler | 页面偏离 HiUI 克制视觉基线，出现未批准的装饰性漂移 | 移除装饰效果，或补齐页型规范 / design brief / governed waiver 证据 | 部分 | 是 |
