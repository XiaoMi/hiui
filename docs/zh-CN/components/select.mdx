...
| showJustSelected    | 是否显示**仅看已选**，只对多选生效                                                                       | boolean                                               | true \| false                                            | false                                                                 |
...
| getOverlayContainer | 菜单渲染父节点。默认渲染到 body 上，如果你遇到菜单滚动定位问题，试试修改为滚动的区域，并相对其定位       | function(triggerNode)                                 | -                                                        | () => document.body                                                   |

> 注意，如果发现下拉菜单跟随页面滚动，或者需要在其他弹层中触发 Select，请尝试使用 getOverlayContainer={triggerNode => triggerNode.parentElement} 将下拉弹层渲染节点固定在触发器的父元素中。
