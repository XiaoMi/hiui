# DatePicker 日期选择器

日期选择器

## 普通模式

import DemoNormal from '../../demo/date-picker/section-normal.jsx'

<DemoNormal />

## 禁用模式

import DemoBan from '../../demo/date-picker/section-ban.jsx'

<DemoBan />

## 范围限制

import DemoBanDate from '../../demo/date-picker/section-ban-date.jsx'

<DemoBanDate />

## 周选择

import DemoWeek from '../../demo/date-picker/section-week.jsx'

<DemoWeek />

## 年选择

import DemoYear from '../../demo/date-picker/section-year.jsx'

<DemoYear />

## 月选择

import DemoMonth from '../../demo/date-picker/section-month.jsx'

<DemoMonth />

## 日期范围选择

import DemoScope from '../../demo/date-picker/section-scope.jsx'

<DemoScope />

## 日期范围选择（范围限制）

import DemoScopeBan from '../../demo/date-picker/section-scope-ban.jsx'

<DemoScopeBan />

## 周范围选择

import DemoWeekScope from '../../demo/date-picker/section-week-scope.jsx'

<DemoWeekScope />

## 日期选范围选择（快捷选项）

import DemoFastCheck from '../../demo/date-picker/section-fast-check.jsx'

<DemoFastCheck />

## 日期 + 时间

import DemoDateTime from '../../demo/date-picker/section-date-time.jsx'

<DemoDateTime />

## 日期/时间段选择

import DemoDateTimeCheck from '../../demo/date-picker/section-date-time-check.jsx'

<DemoDateTimeCheck />

## 日期/时间（范围选择）

import DemoDateTimeScope from '../../demo/date-picker/section-date-time-scope.jsx'

<DemoDateTimeScope />

## 嵌入 Modal

import DemoModal from '../../demo/date-picker/section-modal.jsx'

<DemoModal />

## Props

| 参数        | 说明                              | 类型                                               | 可选值                                                                                                                                       | 默认值 |
| ----------- | --------------------------------- | -------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| type        | 选择器类型                        | string                                             | date 普通日期 <br/> daterange 日期范围<br/> year 年份<br/> month 月份<br/> week 周<br/> weekrange 周范围 <br/> timeperiod 时间段（1.5 新增） | date   |
| value       | 默认显示的日期                    | Date \| string \| number\| DateRange \| undefined \| null | -                                                                                                                                           | null   |
| minDate     | 最小日期                          | Date                                               | null                                                                                                                                         | null   |
| maxDate     | 最大日期                          | Date                                               | null                                                                                                                                         | null   |
| disabled    | 是否禁用输入框                    | boolean                                            | true \| false                                                                                                                                | false  |
| clearable   | 是否可以清空                      | boolean                                            | true \| false                                                                                                                                | true   |
| showTime    | 是否在日期选择器中显示时间选择器     | boolean                                            | true \| false                                                                                                                                     | false  |
| showLunar | 是否在日期选择器中显示农历             | boolean                                            | true \| false                                                                                                                                       | false  |
| altCalendar | 日期中历法信息                      | calendarItem                                    | -                                                                                                                                             | 农历 & 假日  |
| altCalendarPreset | 预置农历信息                  | string                                           | PRCLunar                                                                                                                                       | PRCLunar  |
| dateMarkRender | 日期的右上角标记     | function(currentDate: timestamp, today: timestamp) => React.ReactNode | -                                                                                                                      | 休 \| 班 |
| dateMarkPreset | 预置中国大陆节假日                     | string                                         | PRCHoliday                                                                                                                                     | PRCHoliday |
| shortcuts   | 快捷面板                          | string[]                                              | 近一周, 近一月, 近三月, 近一年                                                                                                               | null   |
| weekOffset  | 周起始<br/>默认周日做为第一列     | number                                             | 0 \| 1                                                                                                                                          | 0      |
| placeholder | 自定义占位符<br/>数组用于范围日期 | string \| string[]                                    | -                                                                                                                                            | -      |
| onChange    | 值变化时的回调函数                | (date: Date \| DateRange, dateStr: string \| DateRange ) => void                                    | -                                                                                                | -      |


## Type
### calendarItem
| 参数     | 说明      | 类型    | 可选值 | 默认值 |
| -------- | ------- | ------- | ------ | ------ |
| date    | 日期     | Date \| string  \| timestamp(ms) | -      | -      |
| text    |  描述信息  | string  |  -  | - |



### DateRange
| 参数     | 说明            | 类型             | 可选值 | 默认值 |
| -------- | --------------- | ---------------- | ------ | ------ |
| start    | 起始时间    | Date \| string \| number \| undefined \| null          | -      | -      |
| end       | 截止时间 | Date \| string \| number \| undefined \| null | -      | -      |

## Methods

`DatePicker.format(date: Date, format: string)`

格式化时间

| 单位        | 字符 | 示例                             |
| ----------- | ---- | -------------------------------- |
| 月          | M    | 1, 2, ..., 12                    |
| &nbsp;      | Mo   | 1st, 2nd, ..., 12th              |
| &nbsp;      | MM   | 01, 02, ..., 12                  |
| &nbsp;      | MMM  | Jan, Feb, ..., Dec               |
| &nbsp;      | MMMM | January, February, ..., December |
| 季度        | Q    | 1, 2, 3, 4                       |
| &nbsp;      | Qo   | 1st, 2nd, 3rd, 4th               |
| 天/月       | D    | 1, 2, ..., 31                    |
| &nbsp;      | Do   | 1st, 2nd, ..., 31st              |
| &nbsp;      | DD   | 01, 02, ..., 31                  |
| 天/年       | DDD  | 1, 2, ..., 366                   |
| &nbsp;      | DDDo | 1st, 2nd, ..., 366th             |
| &nbsp;      | DDDD | 001, 002, ..., 366               |
| 天/周       | d    | 0, 1, ..., 6                     |
| &nbsp;      | do   | 0th, 1st, ..., 6th               |
| &nbsp;      | dd   | Su, Mo, ..., Sa                  |
| &nbsp;      | ddd  | Sun, Mon, ..., Sat               |
| &nbsp;      | dddd | Sunday, Monday, ..., Saturday    |
| 年          | YY   | 00, 01, ..., 99                  |
| &nbsp;      | YYYY | 1900, 1901, ..., 2099            |
| 上午/下午   | A    | AM, PM                           |
| &nbsp;      | a    | am, pm                           |
| &nbsp;      | aa   | a.m., p.m.                       |
| 小时        | H    | 0, 1, ... 23                     |
| &nbsp;      | HH   | 00, 01, ... 23                   |
| &nbsp;      | h    | 1, 2, ..., 12                    |
| &nbsp;      | hh   | 01, 02, ..., 12                  |
| 分钟        | m    | 0, 1, ..., 59                    |
| &nbsp;      | mm   | 00, 01, ..., 59                  |
| 秒          | s    | 0, 1, ..., 59                    |
| &nbsp;      | ss   | 00, 01, ..., 59                  |
| 秒（1/10）  | S    | 0, 1, ..., 9                     |
| 秒（1/100） | SS   | 00, 01, ..., 99                  |
| 毫秒        | SSS  | 000, 001, ..., 999               |
| 毫秒时间戳  | x    | 512969520900                     |
