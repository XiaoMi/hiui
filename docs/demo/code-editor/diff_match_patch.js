;(function () {
  ...
  diff_match_patch.prototype.diff_main = function (a, b, c, d) {
    typeof d === 'undefined' &&
      (d = this.Diff_Timeout <= 0 ? Number.MAX_VALUE : new Date().getTime() + 1e3 * this.Diff_Timeout)
    if (a == null || b == null) throw Error('Null input. (diff_main)')
    if (a == b) return a ? [[0, a]] : []
    typeof c === 'undefined' && (c = !0)
    var e = c
    var f = this.diff_commonPrefix(a, b)
    c = a.substring(0, f)
    a = a.substring(f)
    b = b.substring(f)
  ...
  diff_match_patch.prototype.diff_bisect_ = function (a, b, c) {
    ...
    j[g + 1] = 0
    i[g + 1] = 0
    for (var k = d - e, q = k % 2 != 0, r = 0, t = 0, p = 0, w = 0, v = 0; v < f && !(new Date().getTime() > c); v++) {
      for (var n = -v + r; n <= v - t; n += 2) {
        ...
        else if (q && ((l = g + k - n), l >= 0 && l < h && i[l] != -1)) {
          var u = d - i[l]
      ...
      for (n = -v + p; n <= v - w; n += 2) {
        ...
        if (u > d) w += 2
          ...
          !q &&
          ((l = g + k - n), l >= 0 && l < h && j[l] != -1 && ((m = j[l]), (s = g + m - l), (u = d - u), m >= u))
        )
  ...
  diff_match_patch.prototype.diff_halfMatch_ = function (a, b) {
    function c(a, b, c) {
      for (
        ...
        var s = f.diff_commonSuffix(a.substring(0, c), b.substring(0, e))
        g.length < s + m &&
    ...
    var f = this
    var g = c(d, e, Math.ceil(d.length / 4))
    var d = c(d, e, Math.ceil(d.length / 2))
    var h
    if (!g && !d) return null
  ...
  diff_match_patch.prototype.diff_cleanupSemantic = function (a) {
    for (var b = !1, c = [], d = 0, e = null, f = 0, g = 0, h = 0, j = 0, i = 0; f < a.length; )
      a[f][0] == 0
        ...
        : (a[f][0] == 1 ? (j += a[f][1].length) : (i += a[f][1].length),
          e &&
  ...
  diff_match_patch.prototype.diff_cleanupSemanticLossless = function (a) {
    function b(a, b) {
      ...
      var d = b.charAt(0)
      var e = c.match(diff_match_patch.nonAlphaNumericRegex_)
      var f = d.match(diff_match_patch.nonAlphaNumericRegex_)
      var g = e && c.match(diff_match_patch.whitespaceRegex_)
      var h = f && d.match(diff_match_patch.whitespaceRegex_)
      var c = g && c.match(diff_match_patch.linebreakRegex_)
      var d = h && d.match(diff_match_patch.linebreakRegex_)
      var i = c && a.match(diff_match_patch.blanklineEndRegex_)
      var j = d && b.match(diff_match_patch.blanklineStartRegex_)
    ...
    for (var c = 1; c < a.length - 1; ) {
      if (a[c - 1][0] == 0 && a[c + 1][0] == 0) {
        ...
        var e = a[c][1]
        var f = a[c + 1][1]
        var g = this.diff_commonSuffix(d, e)
        ...
        for (var g = d, h = e, j = f, i = b(d, e) + b(e, f); e.charAt(0) === f.charAt(0); ) {
          var d = d + e.charAt(0)
          var e = e.substring(1) + f.charAt(0)
          var f = f.substring(1)
          var k = b(d, e) + b(e, f)
          k >= i && ((i = k), (g = d), (h = e), (j = f))
        }
  ...
  diff_match_patch.prototype.diff_cleanupEfficiency = function (a) {
    for (var b = !1, c = [], d = 0, e = null, f = 0, g = !1, h = !1, j = !1, i = !1; f < a.length; ) {
      if (a[f][0] == 0)
        ...
        (a[f][0] == -1 ? (i = !0) : (j = !0),
        e && ((g && h && j && i) || (e.length < this.Diff_EditCost / 2 && g + h + j + i == 3)))
      )
        a.splice(c[d - 1], 0, [-1, e]),
          (a[c[d - 1] + 1][0] = 1),
          d--,
          (e = null),
          g && h ? ((j = i = !0), (d = 0)) : (d--, (f = d > 0 ? c[d - 1] : -1), (j = i = !1)),
          (b = !0)
      f++
    }
    b && this.diff_cleanupMerge(a)
  ...
  diff_match_patch.prototype.diff_prettyHtml = function (a) {
    for (var b = [], c = /&/g, d = /</g, e = />/g, f = /\n/g, g = 0; g < a.length; g++) {
      var h = a[g][0]
      var j = a[g][1]
      var j = j.replace(c, '&amp;').replace(d, '&lt;').replace(e, '&gt;').replace(f, '&para;<br>')
      switch (h) {
        case 1:
  ...
  diff_match_patch.prototype.match_bitap_ = function (a, b, c) {
    ...
    for (var j = 1 << (b.length - 1), h = -1, i, k, q = b.length + a.length, r, t = 0; t < b.length; t++) {
      ...
      for (k[p + 1] = (1 << t) - 1; p >= i; p--) {
        ...
        k[p] =
          t === 0 ? ((k[p + 1] << 1) | 1) & w : (((k[p + 1] << 1) | 1) & w) | (((r[p + 1] | r[p]) << 1) | 1) | r[p + 1]
  ...
  diff_match_patch.prototype.patch_make = function (a, b, c) {
    ...
    else if (a && typeof a === 'object' && typeof b === 'undefined' && typeof c === 'undefined')
      (b = a), (d = this.diff_text1(b))
    else if (typeof a === 'string' && b && typeof b === 'object' && typeof c === 'undefined') d = a
    else if (typeof a === 'string' && typeof b === 'string' && c && typeof c === 'object') (d = a), (b = c)
  ...
  diff_match_patch.prototype.patch_apply = function (a, b) {
    ...
    for (var d = 0, e = [], f = 0; f < a.length; f++) {
      var g = a[f].start2 + d
      var h = this.diff_text1(a[f].diffs)
      var j
      var i = -1
      ...
      if (j == -1) (e[f] = !1), (d -= a[f].length2 - a[f].length1)
      else if (
        ((e[f] = !0),
        (d = j - g),
        (g = i == -1 ? b.substring(j, j + h.length) : b.substring(j, i + this.Match_MaxBits)),
        h == g)
      )
        ...
        for (var h = 0, k, i = 0; i < a[f].diffs.length; i++) {
          var q = a[f].diffs[i]
          q[0] !== 0 && (k = this.diff_xIndex(g, h))
          q[0] === 1
            ? (b = b.substring(0, j + k) + q[1] + b.substring(j + k))
            : q[0] === -1 && (b = b.substring(0, j + k) + b.substring(j + this.diff_xIndex(g, h + q[1].length)))
          q[0] !== -1 && (h += q[1].length)
        }
      }
    }
      ...
      if (!(a[c].length1 <= b)) {
        ...
        for (var e = d.start1, f = d.start2, g = ''; d.diffs.length !== 0; ) {
          ...
          i !== '' &&
            ((h.length1 += i.length),
            (h.length2 += i.length),
            h.diffs.length !== 0 && h.diffs[h.diffs.length - 1][0] === 0
