# Stepper 步骤

步骤

## 基础用法

import DemoBase from './section-base.jsx'

<DemoBase />

## 图标用法

stepper 图标用法传入 src 值

import DemoIcon from './section-icon.jsx'

<DemoIcon />

## 上下结构

stepper 上下结构只需加入 up=true；只有上下结构有 text 描述

import DemoUpDown from './section-up-down.jsx'

<DemoUpDown />

## 上下结构的图标用法

stepper 上下结构的图标用法需要加入 up=true，并在数值列表中添加 src 值

import DemoUpDownIcon from './section-up-down-icon.jsx'

<DemoUpDownIcon />

## 竖直方向

stepper 竖直方向只需传入属性 vertical=true

import DemoVertical from './section-vertical.jsx'

<DemoVertical />

## 竖直方向图标用法

stepper 竖直方向只需传入属性 vertical=true

import DemoVerticalIcon from './section-vertical-icon.jsx'

<DemoVerticalIcon />

## Props

| 参数       | 说明                            | 类型        | 可选值                 | 默认值     |
| ---------- | ------------------------------- | ----------- | ---------------------- | ---------- |
| className  | 样式名                          | string      | -                      | -          |
| data       | 步骤数据项                      | DataItem [] | -                      | -          |
| current    | 当前步骤位置索引，从 0 开始计数 | number      | -                      | -          |
| placement  | 位置                            | string      | vertical \| horizontal | horizontal |
| itemLayout | 步骤点的布局方式                | string      | vertical \| horizontal | false      |

## Type

**_DateItem_**

| 参数    | 说明        | 类型                | 可选值 | 默认值 |
| ------- | ----------- | ------------------- | ------ | ------ |
| title   | 显示文案    | string \| ReactNode | -      | -      |
| content | 内容文案    | string \| ReactNode | -      | -      |
| icon    | 自定义 icon | string \| ReactNode | -      | -      |
